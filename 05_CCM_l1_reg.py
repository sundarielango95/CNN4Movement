...
#    eye = Concatenate(axis=-1)([input_left, input_right])
    ...
    # Left layer 2
    left_vc2 = Conv2D(4, (5, 5), strides=(1,1),padding='same',activity_regularizer=l1(0.00005))(VC1_l)
    ...
    # Right layer 2
    right_vc2 = Conv2D(4, (5, 5), strides=(1,1),padding='same',activity_regularizer=l1(0.00005))(VC1_r)
...
def train_test_shuffle_split(I1,I2,I3,I4,Y):
    ...
    return inp1,inp2,inp3,inp4,y,test_ind
...
for num_obj in num_object:
    ...
    for excel_directory in excel_directories:
        ...
        input_images_left_mod,input_images_right_mod,hc_l_inputs_mod,hc_r_inputs_mod,MN_mod,test_indices = train_test_shuffle_split(input_images_left_num_obj,input_images_right_num_obj,hc_l_inputs,hc_r_inputs,MN)
...
#    percent = 80
        ...
        input_images_left_train,input_images_right_train,hc_l_inputs_train,hc_r_inputs_train,MN_train,test_indices = train_test_shuffle_split(input_images_left_all,input_images_right_all,hc_l_inputs_all,hc_r_inputs_all,MN_all)    
#        # Build and train model
        Motor_Cortex = CCM()
        Motor_Cortex.compile(loss="mse", optimizer="adam")
        filepath ="Healthy_Subject_"+str(nEp+1)+"_"+str(100-percent)+"_percent_overlap_w_reg_l1_00005.h5"
        checkpoint1 = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
        callbacks_list = [checkpoint1]   
        mmm = Motor_Cortex.fit([input_images_left_train,input_images_right_train,hc_l_inputs_train,hc_r_inputs_train], MN_train, epochs = 20,batch_size = 10, validation_split = 0.2,callbacks=callbacks_list)
        with open("Healthy_Subject_"+str(nEp+1)+"_"+str(100-percent)+"_percent_overlap_FM_indices_w_reg_l1_00005.txt", "wb") as fp:
                pickle.dump(ft_arr, fp)
        with open("Healthy_Subject_"+str(nEp+1)+"_"+str(100-percent)+"_percent_overlap_FC_indices_w_reg_l1_00005.txt", "wb") as fp:
                pickle.dump(fc_arr, fp)
#        with open("Healthy_Subject_"+str(nEp+3)+"_"+str(100-percent)+"_percent_overlap_FM_indices.txt", "rb") as fp:
#            ft_arr = pickle.load(fp)
#        with open("Healthy_Subject_"+str(nEp+3)+"_"+str(100-percent)+"_percent_overlap_FC_indices.txt", "rb") as fp:
#            fc_arr = pickle.load(fp)
#        model = load_model("Healthy_Subject_"+str(nEp+3)+"_"+str(100-percent)+"_percent_overlap.h5",custom_objects={'feature_map_percent_lesion': feature_map_percent_lesion,'fully_connected_percent_lesion': fully_connected_percent_lesion,'page_dr_shepherd_FC_left':page_dr_shepherd_FC_left,'page_dr_shepherd_FC_right':page_dr_shepherd_FC_right})
#        print("Model Loaded")
#        wts = model.get_weights()
#        patient = CCM()
#        patient.set_weights(wts)
#        patient.compile(loss="mse", optimizer="adam")
#        filepath ="E:/Sundari/Sundari/Data/CNN for Stroke/CNN_model_Stroke/V2/Models/Healthy_Models/CCM/Val_Split_0.2/Same_models_More_epochs/From_Epochs_30/Epochs_40/Healthy_Subject_"+str(nEp+3)+"_"+str(100-percent)+"_percent_overlap.h5"
#        checkpoint1 = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
#        callbacks_list = [checkpoint1]   
#        patient.fit([input_images_left_train,input_images_right_train,hc_l_inputs_train,hc_r_inputs_train], MN_train, epochs = 10,batch_size = 10,validation_split = 0.2,callbacks=callbacks_list)
